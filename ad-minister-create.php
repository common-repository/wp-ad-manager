...
<form name="post" method="POST" action="tools.php?page=<?php echo dirname(plugin_basename (__FILE__)); ?>&tab=upload&action=edit" id="post">
<div class="wrap">
<?php 
	if ($_GET['action'] == 'edit') { 
		$stats = get_option('wp-ad-manager_stats');
		...
		$url = get_option('siteurl') . '/wp-admin/edit.php?page=' . dirname(plugin_basename (__FILE__)) . '&tab=upload&action=edit&id=' . $value['id'] . '';
?>
...
<?php } ?>
	...
	<div style="padding: 10px; margin-left: 170px;">
					<input type="hidden" name="page" value="<?php echo dirname(plugin_basename (__FILE__)); ?>/ad-minister.php">
				...
				<?php if ($_GET['action']) : ?>
				<p>
					<form method="POST" action="tools.php?page=<?php echo dirname(plugin_basename (__FILE__)); ?>&tab=upload" onsubmit="return confirm('<?php _e('Are you sure you want to delete this content?', 'ad-minister'); ?>');">
					<input type="hidden" name="page" value="<?php echo dirname(plugin_basename (__FILE__)); ?>/ad-minister.php">
