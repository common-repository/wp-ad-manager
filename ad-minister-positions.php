<?php
	...
	*/
	...
	if ($_POST['action'] == 'save') {
		...
		//if user is trying to create a position with a name that already exists throw an error.
		if ( ( $_POST['edit_position'] != $name ) && array_key_exists($name, $positions) ) {
			echo '<div id="message" class="updated fade"><p><strong>' . __('That position name already exists!') . '</strong></p></div>';			
		} else {
			//if this is a new widget position
			if(!array_key_exists($name, $positions)){
				//sanitize name
				$name = wpadm_sanitize_widget_name($name);
				$i='';
				//make unique
				while(array_key_exists( $name . $i , $positions)){
					$i++;
				}
				$name = $name . $i;
				$positions[$name]['type'] = 'widget';
			}
			//set position
			$positions[$name]['position'] = $name;
			//get the rest of stuff and sanitize it
			$positions[$name]['description'] = wp_kses($_POST['description'], array());
			//should we sanitize these? >>
...
<?php $position = ($key = $_GET['key']) ? $positions[$key] : array(); ?>
    ...
    <form method="POST" action="tools.php?page=<?php echo dirname(plugin_basename (__FILE__)); ?>&tab=positions">
...
<?php } else if ($_GET['action'] == 'delete') {
	...
	$nbr = 0;
	foreach ((array)$content as $con) if ($con['position'] == $key) $nbr++;
...
<form action="tools.php?page=<?php echo dirname(plugin_basename (__FILE__)); ?>&tab=positions" method="post">
	...
	<table width="100%">
		<tr>
		<td><input type='button' class="button" value='<?php _e('No', 'ad-minister'); ?>' onclick="self.location='<?php echo get_option('siteurl'); ?>/wp-admin/tools.php?page=<?php echo dirname(plugin_basename (__FILE__)); ?>&amp;tab=positions'" /></td>
