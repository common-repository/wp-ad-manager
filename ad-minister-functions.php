...
function administer_main() {
	...
	$stats = get_option('wp-ad-manager_stats');
	$content = get_option('wp-ad-manager_content');
	$positions = get_option('wp-ad-manager_positions');
	...
	<ul class="tabs">
		<?php $url = 'tools.php?page='.dirname(plugin_basename (__FILE__));  ?>
...
function administer_load_widgets () {
	...
	foreach ($positions as $position) {
		if ($position['type'] == 'widget') {
			$widget_name = wpadm_sanitize_widget_name($position['position']);
			eval('
			class ' . $widget_name . ' extends WP_Widget {
				function ' . $widget_name . '() {
					parent::WP_Widget(false, $name = "' . $widget_name . '");
				}
				...
				function widget($args, $instance) {		
					...
					echo $instance["title"];
					echo $after_title;
					administer_display_position("' . $position['position'] . '");
					echo $after_widget; 
				}
			}
...
function administer_template_action ($args) {
	...
	if ($edit_position) {
		...
		update_option('wp-ad-manager_positions', $positions);	
	}
...
function administer_init_impressions() {
	global $administer_stats;
	$administer_stats = get_option('wp-ad-manager_stats');

	// Set up stats if there aren't any
	if (!is_array($administer_stats)) {
		$administer_stats = array();
		update_option('wp-ad-manager_stats', $administer_stats);
	}
}
...
function administer_stats ($options = array()) {
	...
	$contents = get_option('wp-ad-manager_content');
	$positions = get_option('wp-ad-manager_positions');
	$stats = get_option('wp-ad-manager_stats');
	if (!is_array($stats))
		$stats = array();
	...
	if (is_admin())
		$link = get_option('siteurl') . '/wp-admin/tools.php?page='.dirname(plugin_basename (__FILE__));
	else
		$link = get_page_link() . '?administer=view';
	
	$table = array();
	
	foreach (array_keys((array)$contents) as $i) {
...
function wpadm_sanitize_widget_name($name = ''){
	$name = preg_replace('/(\s){2,}/i', ' ', $name);
	$name = str_replace(' ', '_', $name);
	$name = preg_replace('/[^a-zA-Z0-9_]/', '', $name);
	if( is_numeric($name[0]) || $name[0] == '_' )
		$name = 'widget_'.$name;
	$methods = get_class_methods('WP_Widget');
	if(in_array($name, $methods))
		$name = 'widget_'.$name;
	$i = '';
	while( class_exists($name . $i) )
		$i++;
	$name = $name . $i;
	return $name;
}
...
/* 
 * administer_upgrade_datastore
 * Take current data from old post_meta datastore and move into
 * options datastore
 */

function administer_upgrade_datastore () {
		
		//Load data from post_meta datastore
		...
		//Save data to new datastores
		update_option('wp-ad-manager_stats', $stats );
		update_option('wp-ad-manager_content', $content );
		update_option('wp-ad-manager_positions', $positions );
		
		//Remove post-id option
